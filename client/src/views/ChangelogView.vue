...
import { FileClock, Pencil, Save, X } from "lucide-vue-next";
    ...
    <div class="mx-auto max-w-4xl space-y-4">
      ...
      <article v-else-if="mode === 'edit'" class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center gap-2 border-b border-slate-100 px-4 py-2.5">
          <FileClock class="h-4 w-4 text-violet-600" />
      ...
      <template v-else-if="releases.length > 0">
        ...
        >
          <div class="flex items-center gap-2 border-b border-slate-100 px-4 py-3">
            <FileClock class="h-4 w-4 text-violet-600" />
            <h2 class="text-sm font-semibold text-slate-900">{{ release.version }}</h2>
          </div>
          <div
            class="prose prose-sm max-w-none p-4 text-slate-700"
            v-html="release.html"
          />
        </article>
      </template>
